                                          ...
                                          std::istream &preprocessed)
{
    ...
    while (preprocessed >> instr)
    {
        ...
        if (preprocessed.good())
        {
            ...
            tinyram_instruction ti(opcode_values[instr], immflag, des, a1, a2);
            printf("%s %zu %zu %zu %lld --> %zu\n", instr.c_str(), immflag, des, a1, a2, ti.as_dword(ap));
            program.add_instruction(ti);
        }
    }
